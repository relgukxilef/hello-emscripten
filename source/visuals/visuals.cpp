...
visuals::visuals(VkInstance instance, VkSurfaceKHR surface) {
    ...
    check(vkAllocateCommandBuffers(
        device.get(), &command_buffer_info, &draw_command_buffer
    ));

    VkCommandBufferBeginInfo begin_info = {
        .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO,
    };
    check(vkBeginCommandBuffer(draw_command_buffer, &begin_info));
    ...
    check(vkEndCommandBuffer(draw_command_buffer));
}
